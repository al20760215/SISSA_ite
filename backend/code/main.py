...
#endpoint para agregar progreso de estudiante en bimestre
@app.route('/progress-bimester/<int:student_id>', methods=['POST'])
@jwt_required()
def update_progress_bimester(student_id):
    try:
        data = request.get_json()
        
        required_fields = [
            "Compromised_card", "Accepted_card", "Chronogram", "Presentation_card", 
            "Bimester1_Completed", "Bimester2_Completed", "Bimester3_Completed"
        ]
        ...
        cur.execute("""
            INSERT INTO Bimesters (
                StudentID, Compromised_card, Accepted_card, Chronogram, Presentation_card, 
                Bimester1_Completed, Bimester2_Completed, Bimester3_Completed
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (StudentID) DO UPDATE SET 
                Compromised_card = EXCLUDED.Compromised_card,
                Accepted_card = EXCLUDED.Accepted_card,
                Chronogram = EXCLUDED.Chronogram,
                Presentation_card = EXCLUDED.Presentation_card,
                Bimester1_Completed = EXCLUDED.Bimester1_Completed,
                Bimester2_Completed = EXCLUDED.Bimester2_Completed,
                Bimester3_Completed = EXCLUDED.Bimester3_Completed;
        """, (student_id, data["Compromised_card"], data["Accepted_card"], data["Chronogram"], data["Presentation_card"], 
              data["Bimester1_Completed"], data["Bimester2_Completed"], data["Bimester3_Completed"]))
        
        conn.commit()
        
        return jsonify({"message": "Progreso del bimestre actualizado correctamente"}), 200
...
@app.route('/progress-final-report/<int:student_id>', methods=['POST'])
@jwt_required()
def update_progress_final_report(student_id):
    try:
        data = request.get_json()
        
        if "FinalReport_Completed" not in data:
            return jsonify({"error": "Falta el campo FinalReport_Completed"}), 400
        
        conn = get_db_connection()
        cur = conn.cursor()
        
        cur.execute("""
            INSERT INTO Bimesters (StudentID, FinalReport_Completed)
            VALUES (%s, %s)
            ON CONFLICT (StudentID) DO UPDATE SET 
                FinalReport_Completed = EXCLUDED.FinalReport_Completed;
        """, (student_id, data["FinalReport_Completed"]))
        
        conn.commit()
        
        return jsonify({"message": "Reporte final actualizado correctamente"}), 200
